...
wss.on('connection', function connection(ws) {
  ...
  ws.on('message', function message(data: any) {
    ...
    sendAction(response.endpoint, handleWebsocketMessage(data, ws), ws as any)
  })
...
function handleWebsocketMessage(data, ws) {
  ...
  switch (response.endpoint) {
    ...
    case Endpoints.JOIN_ROOM:
      ...
      ;(rooms[new CodeModel(response.body.code).code] as Room).addPlayer(
        new Player(new CustomNameModel(response.body.name).name, ws)
      )
